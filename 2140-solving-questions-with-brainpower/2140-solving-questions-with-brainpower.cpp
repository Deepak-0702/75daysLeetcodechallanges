...
long long helper(int idx,vector<vector<int>>&questions,vector<long long>&dp){
    ...
    //brainpower repersents how much index u skip
    long long solve=questions[idx][0]+helper(idx+questions[idx][1]+1,questions,dp);
    long long skip=helper(idx+1,questions,dp);
